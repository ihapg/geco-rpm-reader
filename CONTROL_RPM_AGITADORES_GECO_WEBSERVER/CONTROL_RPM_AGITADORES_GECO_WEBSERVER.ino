...
void checkRPM() {
  if (rpm1 > 0) {
    int timeDif = (micros() - lastUpdateTime) / 1000000;
    if (timeDif >= 12) {
      rpm1 = 0;
      Serial.print("RPM Sensor 1 reset; Time Dif: ");
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          client.println(" s</div>");
          client.print("          <div id=\"timer3\" style=\"color:#b22222;margin:10px\">Time difference: ");
          client.print((micros() - lastUpdateTime) / 1000000);
          client.println(" s</div>");
          //==============================================================================
